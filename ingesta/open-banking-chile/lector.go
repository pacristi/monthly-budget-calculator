...
func LeerScraper(jsonPath string) ([]movimientos.MovimientoBruto, error) {
	dtos, err := NewClient(jsonPath).Fetch()
	if err != nil {
		return nil, fmt.Errorf("leyendo JSON %s: %w", jsonPath, err)
	}

	out := make([]movimientos.MovimientoBruto, 0, len(dtos))
	for _, d := range dtos {
...
func scraperABruto(d MovimientoDTO) (movimientos.MovimientoBruto, error) {
	fecha, err := time.Parse("02-01-2006", d.Fecha)
	if err != nil {
		return movimientos.MovimientoBruto{}, fmt.Errorf("fecha %q no parseable: %w", d.Fecha, err)
	}

	raw, err := dtoAMap(d)
	if err != nil {
		return movimientos.MovimientoBruto{}, err
	}
